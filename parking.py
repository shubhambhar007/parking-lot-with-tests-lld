...
if __name__ == "__main__":
    # very simple smoke-test for a 2-floor, 2×4 layout
    class DummyHelper:
        def get_spot_id(self, f, r, c):
            return f"{f}-{r}-{c}"
    ...
    helper = DummyHelper()
    sol = Solution()
    sol.init(helper, parking_layout)

    # park a 2-wheeler
    spot = sol.park(2, "BIKE1", "TICKET1")
    print("Parked BIKE1 at:", spot)

    # check free spots on floor 0 for 2-wheelers
    free2 = sol.get_free_spots_count(0, 2)
    print("Free 2-wheeler spots on floor 0:", free2)

    # remove the same vehicle
    status = sol.remove_vehicle(spot, "BIKE1", "TICKET1")
    print("Removal status (201=success):", status)
